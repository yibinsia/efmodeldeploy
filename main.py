...
if __name__ == "__main__":
    ...
    if is_windows:
        logging.info("Detected Windows environment. Starting with Uvicorn...")
        uvicorn.run("main:app", host="0.0.0.0", port=int(os.getenv("PORT", "8080")), reload=True)
    else:
        logging.info("Detected non-Windows environment. Starting with Gunicorn...")
        port = os.getenv("PORT", "8080")
        workers = cpu_count() * 2 + 1
        cmd = f"gunicorn main:app --bind 0.0.0.0:{port} --workers {workers} --worker-class uvicorn.workers.UvicornWorker --timeout 120"
        os.system(cmd)